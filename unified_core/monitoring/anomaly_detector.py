...
class AnomalyDetector:
    ...
    def add_mitigation_handler(
        self, severity: Severity, handler: Callable[[Anomaly], None]
    ):
    ...
    def register_metric(
        ...
        """Register a metric and check rules if interval reached."""
        if not self.cfg.enabled:
            return Metric(name=name, value=value)

        metric = self.registry.register(name, value, context, tensor)
        return metric
    ...
    def check(self, force: bool = False) -> List[Anomaly]:
        ...
        Args:
            force: Check even if not at check interval
    ...
    def _handle_anomaly(self, anomaly: Anomaly):
        ...
        if anomaly.severity == Severity.CRITICAL and self.cfg.halt_on_critical:
            self._halted = True
...
def check_tensor_health(
    ...
    Returns an Anomaly if NaN/Inf detected, else None.
    """
    with torch.no_grad():
